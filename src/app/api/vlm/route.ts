...
【分类规则】
类型A（完整题目，有题干+明确问题）：1道题→1个意图，name写具体题目名称（如"比较分数大小"）；多道不同题号→每题1个意图，name用"第1题""第2题"（不带"解答"），每个intent的content只放该题号对应的完整题干文字，严禁把多道题文字混入同一个content；口算/速算合并为1个；同一题号下小题（1.(1)、1.(2)）合并为1个
...
【字段】每意图包含：
- name: 单题和非题目内容必须是"方法/手段+具体目标"的动宾短语，6-12字，如"用通分比较分数大小""理解一次函数图像变化""查拼音"；多题只写"第1题""第2题"，不受动宾短语约束。禁止：太泛的词（如"写作""计算"）、不完整短语（如"忘不了你的"）、名词短语（如"比较大小的方法"）、口语化表达（如"算一下"）
